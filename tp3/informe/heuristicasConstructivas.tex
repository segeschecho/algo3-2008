...
Antes de comenzar, realizamos una limpieza del grafo, para no considerar a los nodos que tendran grado nulo. Como comentamos en el apartado \ref{sacoNulos}, esto tiene un costo $O(V_1+V_2+m)$ donde $V_i$ es la cantidad de nodos de partición i sin filtrar y m la cantidad de ejes. Este costo se deberá sumar a la complejidad total.
...
Este ordenamiento tiene complejidad.  $O(cantMovilesPi*log(cantMovilesPi))$.Luego de ordenarlos, tomamos el primero de la secuencia.Luego estos pasos tienen un costo $O(cantMovilesPi*log(cantMovilesPi))$, siendo Pi la partici'on sobre la cual estamos agregando el nodo.%iteramos la secuencia hasta que el grado maximo sea mayor al grado del nodo en la posición que estamos mirando y de estos nodos nos quedamos con uno, esto nos cuesta $O(cantMovilesPi)$. Luego estos pasos tienen un costo $O(cantMovilesPi + cantMovilesPi*log(cantMovilesPi))$, siendo Pi la partici'on sobre la cual estamos agregando el nodo.
...
\subsection{Comparación con la heuristica trivial}
La heuristica que elegimos se comporto bien en cuanto a tiempo y cruces comparandola con las otras heuristicas planteadas. Sin embargo, nos pareció interesante ver si nuestra heuristica era por lo menos mejor que la heuristica trivial o heuristica del avestruz \footnote{el metodo del avestruz consiste en, frente a un problema, esconder la cabeza bajo tierra y no hacer nada, de este modo, como el dibujo se devuelve como vino, la analogía es válida} consistente en no hacer nada y devolver la entrada como vino (con los nodos nuevos agregados al final). 
...
\centering
\subfigure[]{
\includegraphics[scale=0.5]{./graficos/constructivaVSavestruz/AvestruzVSConstructiva1.png}}
\subfigure[]{
\includegraphics[scale=0.5]{./graficos/constructivaVSavestruz/AvestruzVSConstructiva2.png} }
\end{figure}
\begin{figure}[H]
\centering
\subfigure[]{
\includegraphics[scale=0.6]{./graficos/constructivaVSavestruz/AvestruzVSConstructiva3.png} }
\caption{Comparación entre inserción greedy de nodos y heuristica optimista (cantidad de cruces del resultados)}
\setcounter{subfigure}{0}
\end{figure}

Como vemos, nuestra heuristica se muestra considerablemente mejor que el acercamiento optimista, y esta mejora se hace mas clara en grafos densos y con un número alto de nodos.

\subsection{Tiempo de ejecución}
Para observar el comportamiento decidimos medir los tiempos variando distintos parametros en los grafos que utilizamos como entrada para la heuristica. La idea era ver que influencia tenían en el tiempo de ejecución del algoritmo la variación de la cantidad de nodos, ejes y nodos libres o nuevos.

Nuestra primer experiencia consitió en utilizar grafos aleatorios con una cantidad n de nodos en cada partición. Variamos este n con distintos porcentajes de nodos libres y ejes. Los resultados son los siguientes:
...
\end{figure}
Los porcentajes indican la proporción de ejes con respecto al grafo completo (n*n es 100 \%) y la cantidad de nodos moviles
Lo primero que observamos es que la complejidad teorica se acerca considerablemente a los resultados empiricos. Por otro lado, se puede notar también a simple vista es que no solo la cantidad de nodos tiene una fuerte influencia en el tiempo de ejecución, sino que también la densidad del grafo asi como también la cantidad de nodos moviles agregados. Esto es de esperarse ya que como vimos en el analisis de complejidad (sección \ref{complejidadDeLaConstructiva}) estos factores son tenidos en cuenta en el analisis de complejidad del algoritmo. Y esto tiene sentido, ya que mas ejes hace mas costoso el calculo de cruces y mas nodos moviles aumenta el número de pasos hasta construir una solución.

Las siguientes expericiencias se hicieron con la intención de observar la influencia, antes comentada, de la cantidad de ejes y de nodos moviles. Las experiencias se realizaron en grafos de 100 nodos por partición. A continuación presentamos los resultados:

\begin{figure}[H]
\centering
\setcounter{subfigure}{0}
\includegraphics[scale=0.4]{./graficos/benchmarkConstructiva/100NodosAumentandoMoviles.png}
\caption{Tiempo en función de la cantidad de nodos moviles en cada particion}
\end{figure}

Los porcentajes indican la proporcion de ejes en el grafo con respecto al completo.

Como vemos, hay una influencia muy grande en la cantidad de nodos moviles. A medida que aumenta la cantidad aumenta el tiempo, sin embargo, observamos que a partir de un 50\% de nodos moviles, el crecimiento parece desacelerarse paulatinamente. En principio no tenemos ninguna hipotesis que pueda explicar esto, no obstante la tendencia es clara en las tres graficas. 
%FIXME: no tenemos hipotesis?

\begin{figure}[H]
\centering
\setcounter{subfigure}{0}
\includegraphics[scale=0.4]{./graficos/benchmarkConstructiva/100NodosAumentandoEjes.png}
...
Lo que observamos es que una mayor cantidad de ejes desemboca en un mayor tiempo de ejecución. El tiempo de ejecución tiene un aspecto lineal, lo cual se condice con el analisis teorico, ya que la complejidad quedaba lineal como función de m. 

A modo de conclusión, podemos decir que los tiempos de ejecución obtenidos empiricamente, reflejaron a los obtenidos en el analisis teorico de la heuristica.
