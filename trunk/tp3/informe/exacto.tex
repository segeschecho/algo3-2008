...
De esta manera, el árbol de \textit{backtracking} que tenemos es el de la figura \ref{arbolBacktracking}:
%FIXME: error en el dibujo, borrar puntos suspensivos en el primer nivel
\begin{figure}
\centering
\setcounter{subfigure}{0}
\includegraphics[scale=0.9]{./figuras/exacto/arbolbt.png}
\caption{Árbol de \textit{backtracking}}\label{arbolBacktracking}
\end{figure}
...
Definimos además definimos $moviles_i$ como la cantidad de nodos nuevos, ya filtrados en la partición i. Por otro lado
...
costo $O(v_{1}^2*v_{max})$, ya que se efectúan $O(v_{1}^2)$ cálculos y cada cálculo tiene costo $O(v_{max})$ como
...
la tabla para la segunda partición. Esto tiene un orden $O(v_2^2*v_{max})$.
...
$O(\frac{v_1!}{fijos_1!}((m+v_{1}^2)*moviles_1+ \frac{(v_2)!}{fijos_2!} *((m+v_{2}^2)*moviles_2 + (v_1+v_2)) + v_2*v_{max}) + V_1 + V_2 ) \subseteq O(t!(t! + P(t)))$, con P(t) un polinomio en t. Luego el algoritmo es $O(t!^2)$ 
