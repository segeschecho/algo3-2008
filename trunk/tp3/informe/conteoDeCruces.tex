...
Para calcular crucesEntre se puede utilizar el algoritmo antes descripto para el conteo de cruces en general, simulando una partición que solo contenga a los nodos $a$ y $b$. En este caso, como la partición mas chica tiene 2 nodos (o menos si una partición era solo un nodo), , resulta que el algoritmo puede calcular los cruces en $O(\sharp v_2 + m_{a,b})$ con $m_{a,b}$ la cantidad de ejes incidentes a $a$ y a $b$ y $v_2$ es la otra partición. 
...
Hecho esto, aplicar el algoritmo de conteo de cruces para $\pi_3$ y como $pi_3$ tiene 2 nodos, el algoritmo es $Calcular Bien$ (nuevamente, bajo hipotesis de que no hay nodos aislados).  
