...
Entonces si ordenamos los ejes de esa manera y después contamos las inversiones, podemos obtener el numero de cruces. Para ordenar a los ejes se puede utilizar radix sort, suponiendo que los ejes tienen como componentes las posiciones relativas de los ejes (o que estas se pueden obtener), lo cual se puede calcular en caso de que esa información no este disponible con un costo de $n_1 + n_2 + m $, con $n_i$ la cantidad de nodos de la partici'on i, obteniendo los indices de los nodos en cada particion y luego usando esta información ``traducir'' los ejes; y luego considerar el arreglo que tiene a las segundas componentes de los ejes como elementos, aplicar insertion sort y por cada cambio de posición que se hace para un elemento, contar una inversión y por lo tanto un cruce. Esto último tendría un costo $O(m+c)$, con c la cantidad de cruces. En el peor caso, todos los ejes se cruzan por lo que tendríamos un costo $O(m^2)$ nuevamente.
...
Para calcular crucesEntre se puede utilizar el algoritmo antes descripto para el conteo de cruces en general, simulando una partición que solo contenga a los nodos $a$ y $b$. En este caso, como la partición tiene 2 nodos, y si ademas tenemos en cuenta que filtramos los nodos aislados y que por lo tanto vale que $m>\sharp v1\wedge m > \sharp v2$, resulta que el algoritmo puede calcular los cruces en $O(m_{a,b})$ con $m_{a,b}$ la cantidad de ejes incidentes a $a$ y a $b$. 
...
Para averiguar $\sum_{k=i+1}^{j-1}{(CrucesEntre(v_i,v_k,\rho))}$, se puede considerar un $\pi_3 = <v_i,w>$ donde w consiste en ``juntar'' todos los nodos $v_k$ con $k = i +1 ... j-1$, dandole todos sus ejes (podríamos obtener un multigrafo bipartito). Hecho esto, aplicar el algoritmo de conteo de cruces para $\pi_3$ y como $pi_3$ tiene 2 nodos, el algoritmo es $O(m)$ (nuevamente, bajo hipotesis de que no hay nodos aislados).  
