...
\begin{algorithmic}[1]
\PARAMS{un dibujo original, un alfa, un booleano randPos}
...
\STATE nodo $\leftarrow$ elegir uno entre los nodos a poner que superen un grado alfa-maximo
...
\STATE posCandidatas = ultima posicion
...
\IF{ cruces $<$ mejorCruces}
	...
	\STATE posCandidatas $\leftarrow$ la posicion donde esta ahora
\ENDIF
\STATE crucesPreSwap $\leftarrow$ crucesPostSwap
\ENDWHILE
\IF{randPos = true}
	\STATE poner al nodo en una de las posiciones candidatas de forma aleatoria
\ELSE
	\STATE poner al nodo en la primer posicion candidata hallada
\ENDIF
...
Veamos ahora cuanto nos cuesta elegir un nodo entre los nodos a agregar que supere un grado alfa-m'aximo. Primero ordenamos de forma creciente (con sort de STL) los nodos a agregar por su grado parcial, es decir considerando solamente los ejes que van del nodo hacia un nodo fijo o del nodo hacia un nodo que esta ya fue agregado. Este ordenamiento tiene complejidad $O(cantMovilesPi*log(cantMovilesPi))$.Luego de ordenarlos, iteramos la secuencia hasta que el grado maximo multiplicado por alfa sea mayor al grado del nodo actualmente iterando, esto nos cuesta $O(cantMovilesPi)$. Todo esto tiene costo $O(cantMovilesPi + cantMovilesPi*log(cantMovilesPi))$, siendo Pi la partici'on sobre la cual estamos agregando el nodo.

Ahora buscamos una de las mejores posiciones en la partici'on para insertar este nodo. Primero actualizamos la lista de adyacencias parcial para incorporar las adyacencias del nodo que voy a agregar (esto es necesario pues las funciones de conteo de cruces se usan dentro del subgrafo fijo y por tanto para que tengan en cuenta al nodo a agregar, es necesario completarlas con sus ejes). Suponemos que insertamos el nodo al final de la partici'on. Actualizar la lista de adyacencias parcial tiene un costo $O(m)$. Luego contamos los cruces por agregar atraso tiene un costo $O(n+m)$. Ahora swapeamos el nodo con su nodo anterior de la particion y recalculamos los cruces hasta que el nodo a agregar queda primero. Aqu'i podemos aprobechar lo mostrado en el apartado que explica el conteo de cruces, y lo hacemos con costo $O(min(max(vi,m_a,m_b),m_a*m_b))$. Esto, haciendolo por cada posici'on en la partici'on que estamos agregando, tiene un costo de $O(cantModosParticion * min(max(vi,m_a,m_b),m_a*m_b))$. Dado que $max(vi, m_a, m_b) \leq v_{max}$, $min(a, b) \leq a$ o sea $min(v_{max},m_a*m_b)) \leq v_{max}$ y que $cantMnodosParticion \leq v_{max}$ entonces la complejidad de este ciclo nos queda $O(v_{max}^2)$.

Finalmente insertamos el nodo y actualizamos los grados parciales y el vector de posiciones, con un costo de $O(m + cantMnodosPartici'on)$.

Dado que este procedimiento lo hacemos para todos los nodos a agregar, la complejidad resultante es: $O(cantMoviles*(m + n + v_{max}^2 + cantMoviles + cantMovilesP1*log(cantMovilesP1) + cantMovilesP2*log(cantMovilesP2)) + m*log(p_{max})+p_max) \subseteq O(cantMoviles*v_{max}^2 + m*log(fijos_{max})+fijos_{max})$ siendo cantMoviles la cantidad total de nodos a agregar, $v_{max}$ la cantidad de nodos de la partici'on mas grande del dibujo resultante, $m$ la cantidad de ejes del dibujo resultante y $p_{max}$ la cantidad de nodos fijos de la partici'on que m'as nodos fijos tiene.
