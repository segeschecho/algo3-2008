...
\item Con esta información podemos armar el nuevo grafo.
...
A continuación creamos el grafo invertido. Como tenemos la informacion de las relaciones en listas de adyacencia, la construccion del mismo, nos cuesta nuevamente $O(n+m)$.
...
La primer experiencia consistió en medir la cantidad de operaciones en función de ambas variables.

Luego decidimos dejar fija cada una ellas para observar el comportamiento por separado.

Finalmente decidimos hacer una ultima experiencia variando ambas variables, pero manteniendo constante la relación entre ellas, en particular consideramos m = (n-1)*n/2, es decir, generando grafos completos.
...
Finalmente en la grafica de la última experiencia notamos que la cantidad de operaciones crece cuadraticamente. Nosotros esperabamos que eso ocurra, ya que esta forma parabolica se explica en que como tomamos $m=\frac{(n-1)*n}{2}$ resulta que m es $\Theta(n^2)$, entonces O(n + m) es O($n^2$).
